...
export class Player extends GameObject {
    ...
    update(deltaTime) {
        // --- NOWA ZMIANA: SPRAWDZANIE ŚMIERCI GRACZA ---
        if (this.currentHP <= 0) {
            console.log("Gracz zginął! Restart poziomu...");
            this.game.loadLevel(this.game.currentLevelIndex);
            return; // Zakończ aktualizację, aby uniknąć błędów
        }
        // --- KONIEC NOWEJ ZMIANY ---
        ...
        // Logika strzelania
        if (input.keys["Space"] && !this.isCharging && this.canShoot) {
            this.isCharging = true;
            this.chargeStartTime = Date.now();
        } else if (!input.keys["Space"] && this.isCharging) {
            this.isCharging = false;
            this.power = Math.min(Date.now() - this.chargeStartTime, MAX_POWER_CHARGE);
            this.shootArrow();
        }
    ...
    shootArrow() {
        ...
        const arrowPower = this.power / MAX_POWER_CHARGE;
        const dx = this.facingDirection === "right" ? ARROW_SPEED : -ARROW_SPEED;
        const dy = ARROW_VERTICAL_STRENGTH * arrowPower;
